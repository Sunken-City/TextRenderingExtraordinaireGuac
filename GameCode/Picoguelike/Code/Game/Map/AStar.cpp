...
//-----------------------------------------------------------------------------------
bool Path::FindPath(const MapPosition& start, const MapPosition& goal)
{
    ClearPath();
    InitializePathForStep(start, goal);
...
void Path::ClearPath()
{
    ...
    m_openList.clear();
    m_closedList.clear();
    m_resultantPath.clear();
    MapPosition m_currentGoal = -Vector2Int::ONE;
}
