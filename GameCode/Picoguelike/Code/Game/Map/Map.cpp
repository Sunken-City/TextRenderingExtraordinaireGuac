...
//-----------------------------------------------------------------------------------
bool Map::FindFirstAdjacentEmptyCell(Vector2Int& outFoundCell, const Vector2Int& cellToSearchAround)
{
    for (int y = -1; y < 2; ++y)
    {
        for (int x = -1; x < 2; ++x)
        {
            Vector2Int potentialCellLocation = cellToSearchAround + Vector2Int(x, y);
            Cell* foundCell = FindCellAt(potentialCellLocation);
            if (foundCell && !foundCell->IsSolid() && !foundCell->IsOccupied())
            {
                outFoundCell = potentialCellLocation;
                return true;
            }
        }
    }
...
//-----------------------------------------------------------------------------------
void Map::UpdateFOVFrom(const Vector2Int& location, int radius, RaycastTileAction* processTileFunction, void* data)
{
    if (data == nullptr)
    {
        data = (void*)this;
    ...
    for (int y = startY; y < endY; ++y)
    {
        for (int x = startX; x < endX; ++x)
        {
            ...
            MapRaycastCollision result = RaycastAmanatidesWoo(Vector2(location) + Vector2::ONE * 0.5, Vector2(x + 0.5f, y + 0.5f), processTileFunction, data);
        }
    }
...
void Map::FindValidAdjacentPositions(std::vector<PathNode*>& outValidPositionsVector, PathNode* start, const Vector2Int& end)
{
    for (int y = -1; y < 2; ++y)
    {
        for (int x = -1; x < 2; ++x)
        {
            ...
            {
                //We can't warp through diagonal walls.
                if (abs(x) == 1.0f && abs(y) == 1.0f)
                {
                    if (TheGame::instance->m_currentMap->GetCellAt(start->position + Vector2Int(x, 0.0f)).IsSolid() && TheGame::instance->m_currentMap->GetCellAt(start->position + Vector2Int(0.0f, y)).IsSolid())
                    {
                        continue;
                    }
                }
...
//-----------------------------------------------------------------------------------
void Map::MarkTileVisible(Cell* foundCell, void* data)
{
    foundCell->m_isVisible = true;
    Map* currentMap = (Map*)data;
    foundCell->m_knownAs = currentMap->m_characterLegend[foundCell->m_type];
}

//-----------------------------------------------------------------------------------
bool Map::IsAdjacent(const Vector2Int& position1, const Vector2Int& position2)
{
    Vector2Int delta = position2 - position1;
...
//-----------------------------------------------------------------------------------
MapRaycastCollision Map::RaycastAmanatidesWoo(const Vector2& start, const Vector2& end, RaycastTileAction* processTileFunction, void* data /*= nullptr*/)
{
    ...
    while (true)
    {
        if (tOfNextXCrossing < tOfNextYCrossing)
        {
            ...
            processTileFunction(nextFoundCell, data);
